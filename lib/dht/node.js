...
Node.prototype.processGetPeers = function processGetPeers(msg, rinfo) {
  ...
  this.respond(rinfo, msg, {
    token: token,
    values: utils.encodePeers(this.peers[infohash].list)
  });
};
...
Node.prototype.announce = function announce() {
  ...
  Object.keys(this.peers).forEach(function(sid) {
    var infohash = new Buffer(sid, 'hex'),
        port = this.peers[sid].port || 0;
  ...
  if (existing) {
    ...
    peer.once('timeout', function() {
      ...
      if (peers.list.length === 0 && peers.port === 0) {
        delete self.peers[infohash.toString('hex')];
      }
    });

  ...
  async.whilst(function() {
    ...
    async.forEach(nodes, function(node, callback) {
      node.findNode(id, function(err, msg) {
        ...
        if (!msg.a || !Buffer.isBuffer(msg.a.nodes)) return callback(null);

        // Add nodes to known
        var nodes = utils.decodeNodes(msg.a.nodes);
        nodes.forEach(function(node) {
          var remote = self.addNode(node.id, node);
...
Node.prototype.addNode = function addNode(id, rinfo) {
  var node;
  ...
  while (true) {
    ...
    // Add node to bucket
    if (node = bucket.add(id, rinfo)) break;

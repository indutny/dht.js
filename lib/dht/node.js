...
function Node(port) {
  ...
  if (typeof port !== 'object' || port === null || !this.load(port)) {
    this.id = new Buffer(
...
Node.prototype.save = function save() {
  return {
    ...
    port: this.port,
    advertisements: this.advertisements.map(function(key) {
      var hid = new Buffer(key, 'hex'),
          peers = this.getBucket(hid).getPeers(hid);

      return {
        port: peers ? peers.port : 0,
        infohash: key
      };
    }, this),
...
Node.prototype.load = function load(config) {
  ...
  if (!config.id ||
      !Array.isArray(config.advertisements) ||
      !Array.isArray(config.nodes)) {
    return false;
  }

  this.id = new Buffer(config.id, 'hex');
  this.socket.bind(config.port || 0);
  ...
  // Add advertisements
  config.advertisements.forEach(function(ad) {
    this.advertise(new Buffer(ad.infohash, 'hex'), ad.port);
  }, this);

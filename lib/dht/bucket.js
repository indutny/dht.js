...
Bucket.prototype.split = function split() {
  // Split is impossible when start === end
  if (Bucket.compare(this.start, this.end) === 0) {
    return false;
  }
  ...
  for (var i = this.end.length - 2; i >= 0; i -= 2) {
    ...
    word += this.start.readUInt16BE(i);
    var downBit = word & 1;
    word = word >> 1;
    rpos.writeUInt16BE(word, i);

    // Shift one bit below
    if (downBit && i + 4 <= rpos.length) {
      var rdownWord = rpos.readUInt16BE(i + 2),
          ldownWord = lpos.readUInt16BE(i + 2);

      assert(rdownWord === 0x0000);
      assert(ldownWord === 0xffff);

      rpos.writeUInt16BE(0x8000, i + 2);
      lpos.writeUInt16BE(0x7fff, i + 2);
